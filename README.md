...
## *注意事项：*
1、运行demo前，请先在终端cd到工程目录执行pod install，添加依赖库，demo才能加载出广告，``` 为了避免不同平台的依赖库版本可能存在冲突或不同版本可能存在函数被弃用，故建议添加（穿山甲：Ads-CN和优量汇：GDTMobSDK）依赖库时请保持跟ADCDN中添加的版本一致```如：GDTMobSDK添加的是4.11.12版本,如果pod search xxx库，没有找到指定的最新版本，可以执行pod repo update 更新最新的cocoapods；

2、广告拉取失败，禁止多次重试请求广告，避免请求量消耗过大，导致填充率过低，影响系统对您流量的评价从而影响变现效果，得不到广告收益。

3、针对穿山甲和优量汇的依赖包，请保持跟ADCDN依赖的版本号一致，避免兼容性问题。
``` java
platform :ios,'9.0'
target 'YX_AD_demo' do
pod 'GDTMobSDK', '4.12.3'
# 注：穿山甲最新的包改名成Ads-CN,后续版本都在这个包下迭代
pod 'Ads-CN','3.4.2.3'
end
```
``` java
...
## 3. 兼容和历史版本
iOS9.0及以上，最新版本号：V 8.6.2，已经对接过ADCDN的，如需要更新最新版本，直接替换demo中的ADCDN.framework和ADCDN.bundle即可。

| 版本号        | 更新内容 | 更新时间 | 
| --------       | -----   |----- | 
| V8.6.2       |  1、适配最新的穿山甲(v3.4.2.3)和优量汇(v4.12.3)的包，注：穿山甲最新的包改名成Ads-CN,后续版本都在这个包下迭代 |2021-01-19
| V8.6.1       |  1、优化原生广告数据上报 |2020-12-23
...
（2）手动引入广告依赖库，请到官网查看引入方式，添加对应的系统依赖库：
 ...
 [优量汇文档](https://developers.adnet.qq.com/doc/ios/union/union_embed#手动部署)  
